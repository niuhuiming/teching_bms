...
exports.getNoticeInfo = (req, res) => {
    const sqlStr = `select * from r_notice;`
...
exports.addNoticeInfo = (req, res) => {
    const ID = nanoid();
    const sqlStr = `insert into r_notice(id_notice, date_publish, title) values ('${ID}', '${req.body.date_publish.slice(0,10)}', '${req.body.title}');`
    db.query(sqlStr, (err, response) => {
        if (err) {
            console.log('error', err.message);
            return res.send({ code: 0, data: err.message })
...
exports.deleteNoticeInfo = (req, res) => {
    // 删除相关政策
    fs.unlink(`./public/notice/${req.body.title}.doc`, err => { 
        if (err) {
            console.log('[error]', err.message);
        }
    ...
    const sqlStr = `delete from r_notice where id_notice = '${req.body.id}'`
    // console.log(sqlStr);
    db.query(sqlStr, (err, response) => {
        if (err) {
            console.log('[error]', err.message);
...
// 下载政策
exports.downloadNoticeById = (req, res) => {
    // console.log('req.query', req.query);
    // // 删除相关政策
    const sqlStr = `select title from r_notice where id_notice = '${req.query.id_notice}'`
    db.query(sqlStr, (err, response) => {
        if (err) {
            console.log('[error]', err.message);
            return res.send({ code: 0, data: err.message })
        }
        // console.log(response[0].title, path.resolve(__dirname, `../public/notice/${response[0].title}.doc`));
        // 读取PDF文件
        fs.readFile(path.resolve(__dirname, `../public/notice/${response[0].title}.doc`), (err, data) => {
            if (err) {
                console.log('[error]', err.message);
                return res.send({ code: 0, data: err.message })
            }
