...
// 获取政策列表
exports.getCheckInfo = (req, res) => {
    const sqlStr = `select * from r_check;`
...
// 新增政策信息
exports.addCheckInfo = (req, res) => {
    const ID = nanoid();
    const sqlStr = `insert into r_check(id_check, date_publish, title) values ('${ID}', '${req.body.date_publish.slice(0,10)}', '${req.body.title}');`
    db.query(sqlStr, (err, response) => {
        if (err) {
            console.log('error', err.message);
            return res.send({ code: 0, data: err.message })
...
// 删除政策
exports.deleteCheckInfo = (req, res) => {
    // 删除相关政策
    fs.unlink(`./public/check/${req.body.title}.doc`, err => { 
        if (err) {
            console.log('[error]', err.message);
        }
    ...
    const sqlStr = `delete from r_check where id_check = '${req.body.id}'`
    // console.log(sqlStr);
    db.query(sqlStr, (err, response) => {
        if (err) {
            console.log('[error]', err.message);
...
// 下载政策
exports.downloadCheckById = (req, res) => {
    // console.log('req.query', req.query);
    // // 删除相关政策
    const sqlStr = `select title from r_check where id_check = '${req.query.id_check}'`
    db.query(sqlStr, (err, response) => {
        if (err) {
            console.log('[error]', err.message);
            return res.send({ code: 0, data: err.message })
        }
        // console.log(response[0].title, path.resolve(__dirname, `../public/check/${response[0].title}.doc`));
        // 读取PDF文件
        fs.readFile(path.resolve(__dirname, `../public/check/${response[0].title}.doc`), (err, data) => {
            if (err) {
                console.log('[error]', err.message);
                return res.send({ code: 0, data: err.message })
            }
