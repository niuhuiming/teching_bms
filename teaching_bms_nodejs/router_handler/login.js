...
exports.login = (req, res) => {
    ...
    db.query(sqlStr, (err, results) => {
        if (err) {
            ...
            return res.send({ code: 1, data: err.message })
        }
        // 检查是否是管理员
        if (results.length) {
            // 生成token
            ...
            db.query(sqlStr, (err, results) => {
                ...
                if (results.length) {
                    ...
                    // 无此人信息
                    return res.send({ code: 0, data: '账号或密码错误' })
                }
            })
        }
    })
...
exports.getUserInfo = (req, res) => {
    ...
    jwt.verify(token, 'secret', (err, decoded) => {
        if (err) {
            ...
            return res.send({ code: 0, data: '请重新登录' })
        }
        if (decoded.username === 'admin') {
            res.send({ code: 1, data: { role: 'admin' } })
        } else {
            res.send({ code: 1, data: { role: 'user' } })
        }
    })
}
