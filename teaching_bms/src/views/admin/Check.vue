<template>
  <div class="root">
    ...
    <PolicyForm title="新增" path="./public/check/" url="/api/addCheckFile" :visible.sync="showAddForm" @save="handleAddPolicy"/>
  </div>
</template>
    ...
    deletePolicy(p) {
      ...
      this.$confirm("确定要删除吗?", "提示", {
        ...
        type: "warning",
          ...
          axios({
            url: "api/deleteCheckInfo",
            method: "post",
            data: { id: p.id_check, title: p.title },
          }).then(() => {
              this.$message({
                type: "success",
                message: "已删除",
